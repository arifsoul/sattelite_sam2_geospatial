...
class LangSAM:
    ...
    def classify_crop(self, cropped_image, folder_path):
        cropped_array = cv2.cvtColor(cropped_image, cv2.COLOR_RGB2BGR)
        best_score = -1
        best_class = None
        ...
        for filename in list_img:
            if filename.lower().endswith(('.png', '.jpg', '.jpeg')):
                ...
                ref_image_resized = cv2.resize(ref_image, (cropped_array.shape[1], cropped_array.shape[0]))

                # Check dimensions before calculating SSIM
                if cropped_array.shape[0] < 7 or cropped_array.shape[1] < 7 or \
                ref_image_resized.shape[0] < 7 or ref_image_resized.shape[1] < 7:
                    continue  # Skip this comparison if any image is too small

                score, _ = ssim(cropped_array, ref_image_resized, channel_axis=-1, full=True, multichannel=True)

                if score > best_score:
                    best_score = score
                    best_class = os.path.splitext(filename)[0]
                    best_class = best_class.split(" ")[0]

        return best_class

    def is_image_zero(self, image):
        return np.sum(image) == 0

    def get_cropped_image_from_mask(self, image, box, mask):
        ...
        crop_img = np.zeros_like(cropped_image, dtype=image.dtype)

        for c in range(image.shape[2]):
            crop_img[..., c] = cropped_image[..., c] * cropped_mask

        if self.is_image_zero(crop_img):
            print("Cropped image contains all zeros.")
            return None  # Kembalikan None jika gambar kosong

        # **Resizing untuk memastikan lebar 100 piksel**
        target_width = 150
        original_height, original_width = crop_img.shape[:2]

        # Hitung tinggi baru sesuai skala
        aspect_ratio = original_height / original_width
        target_height = int(target_width * aspect_ratio)

        # Resize gambar dengan OpenCV
        resized_crop_img = cv2.resize(crop_img, (target_width, target_height), interpolation=cv2.INTER_LINEAR)
        ...
        **kwargs,
        ...
        if predict_masks:
            for (box, mask) in zip(self.boxes, self.masks):
                box = box.cpu().numpy()  # Convert tensor to numpy
                crop_img = self.get_cropped_image_from_mask(np.array(image_pillow), box, mask)
                species = self.classify_crop(crop_img, folder_path)
                classifications["box"].append(box.tolist())
                classifications["mask"].append(mask.tolist())
                classifications["species"].append(species)
